...
const KMEANS = (() => {
  ...
  function kmeans(vectors, k, opts={}){
    ...
    // KMeans++ seeding (nearest squared Euclidean on unit sphere: 2*(1 - dot))
    const centroids = [];
    const first = randInt(n); centroids.push(X[first]);
    const curD2 = new Float64Array(n);
    for (let i = 0; i < n; i++) {
      const s = dot(X[i], centroids[0]);
      curD2[i] = Math.max(0, 2 * (1 - s));
    }
    for (let c = 1; c < k; c++) {
      let sum = 0; for (let i = 0; i < n; i++) sum += curD2[i];
      if (!isFinite(sum) || sum <= 0) { centroids.push(X[randInt(n)]); continue; }
      let r = Math.random() * sum; let idx = 0;
      for (; idx < n; idx++) { r -= curD2[idx]; if (r <= 0) break; }
      const pick = Math.min(idx, n - 1);
      const next = X[pick];
      centroids.push(next);
      // update curD2 with new centroid
      for (let i = 0; i < n; i++) {
        const s = dot(X[i], next);
